...
#define PREANAL(code)  (KC_A <= (code) && (code) <= KC_Z &&    code != KC_A && code != KC_B && code != KC_D && code != KC_E && code != KC_F && code != KC_I && code != KC_J && code != KC_K && code != KC_L && code != KC_M && code != KC_N && code != KC_O && code != KC_Q && code != KC_R && code != KC_U && code != KC_V && code != KC_Y && code != KC_Z)
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  [BASE] = LAYOUT_1_a(
    ...
    KC_MINS,           KC_COMM, KC_DOT,  KC_SLSH, KC_H,    KC_N,      KC_TILD,               KC_PLUS,      KC_V,    KC_D,    KC_C,    KC_K,     KC_G,    KC_Q,
    CTL_T(KC_ENT),     KC_A,    KC_I,    KC_E,    HN,      LCTL(KC_C),KC_RABK,               KC_LABK,      KC_M,    KC_T,    KC_S,    KC_R,     KC_L,    CTL_T(KC_TAB),
    KC_F5,             KC_QUOT, KC_U,    KC_O,    KC_W,    KC_Y,      KC_CIRC, OSM(MOD_LCTL),KC_HASH,      KC_X,    KC_P,    KC_F,    KC_B,     KC_J,    LT(SYMB,KC_APP),
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
  ...
  if (! IS_MOD(keycode) && keycode != OSM(MOD_LSFT) && record->event.pressed) {
    ...
    if (rep_count > 0) {
      ...
      case KC_X:    unregister_weak_mods(MOD_MASK_CSAG); SEND_STRING("\bprob"); return false;
